...
def _safe_url(url):
    """Garante que a URL tem sslmode=require para o Supabase."""
...
def _conn():
    if USE_PG:
        ...
        return psycopg2.connect(_safe_url(DATABASE_URL),
                                connect_timeout=20,
...
def _init_pg():
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"Verifique DATABASE_URL nos Secrets do Streamlit Cloud.\n"
            f"Formato correto: postgresql://postgres:SENHA@db.XXXX.supabase.co:5432/postgres\n"
            f"Erro original: {e}"
        )
    cur = conn.cursor()
...
def _init_sq():
    ...
    """)
    for col, tbl in [("expense_type TEXT DEFAULT 'extra'","extra_expenses"),("investment_source TEXT DEFAULT 'Renda do mês'","investments"),("due_day INTEGER DEFAULT 30","investments"),("due_day INTEGER DEFAULT 30","insurance"),("due_day INTEGER DEFAULT 30","debts")]:
...
# Income
def get_income(m): return _fetch(f"SELECT * FROM income WHERE month={PH} ORDER BY due_day,id",(m,))
def add_income(m,l,a,d=30):
    rid=_exec(f"INSERT INTO income(month,label,amount,due_day) VALUES({PH},{PH},{PH},{PH})",(m,l,a,d)); _hist("INSERT","income",rid,None)
...
# Fixed
def get_fixed(m): return _fetch(f"SELECT * FROM fixed_expenses WHERE month={PH} ORDER BY due_day,id",(m,))
def add_fixed(m,l,a,d,cat): _exec(f"INSERT INTO fixed_expenses(month,label,amount,due_day,category) VALUES({PH},{PH},{PH},{PH},{PH})",(m,l,a,d,cat))
...
def cc_items_month(month):
    ...
    return result
def add_cc(l,tot,inst,sm,cn="Cartão Principal"): _exec(f"INSERT INTO credit_card_items(label,total_amount,installments,start_month,card_name) VALUES({PH},{PH},{PH},{PH},{PH})",(l,tot,inst,sm,cn))
def del_cc(rid): _exec(f"DELETE FROM credit_card_items WHERE id={PH}",(rid,))

# Extras
def get_extras(m,expense_type=None):
    if expense_type: return _fetch(f"SELECT * FROM extra_expenses WHERE month={PH} AND expense_type={PH} ORDER BY id",(m,expense_type))
    return _fetch(f"SELECT * FROM extra_expenses WHERE month={PH} ORDER BY id",(m,))
def add_extra(m,l,a,cat,method,expense_type='extra'): _exec(f"INSERT INTO extra_expenses(month,label,amount,category,payment_method,expense_type) VALUES({PH},{PH},{PH},{PH},{PH},{PH})",(m,l,a,cat,method,expense_type))
...
def add_sub(l,a,cat,bd,notes=""): _exec(f"INSERT INTO subscriptions(label,amount,category,billing_day,notes) VALUES({PH},{PH},{PH},{PH},{PH})",(l,a,cat,bd,notes))
def toggle_sub(rid):
...
def add_debt(l,tot,rem,mp,rate,due_day=30,notes=""): _exec(f"INSERT INTO debts(label,total_amount,remaining_amount,monthly_payment,interest_rate,due_day,notes) VALUES({PH},{PH},{PH},{PH},{PH},{PH},{PH})",(l,tot,rem,mp,rate,due_day,notes))
def update_debt_remaining(rid,v): _exec(f"UPDATE debts SET remaining_amount={PH} WHERE id={PH}",(max(0,v),rid))
def update_debt_due_day(rid,d): _exec(f"UPDATE debts SET due_day={PH} WHERE id={PH}",(d,rid))
...
def add_insurance(l,pv,mc,cv,due_day=30,notes=""): _exec(f"INSERT INTO insurance(label,provider,monthly_cost,coverage,due_day,notes) VALUES({PH},{PH},{PH},{PH},{PH},{PH})",(l,pv,mc,cv,due_day,notes))
def update_insurance_due_day(rid,d): _exec(f"UPDATE insurance SET due_day={PH} WHERE id={PH}",(d,rid))
...
def add_goal(l,tg,cur=0,dl="",notes=""): _exec(f"INSERT INTO goals(label,target_amount,current_amount,deadline,notes) VALUES({PH},{PH},{PH},{PH},{PH})",(l,tg,cur,dl,notes))
def update_goal(rid,cur): _exec(f"UPDATE goals SET current_amount={PH} WHERE id={PH}",(cur,rid))
...
def add_bill_template(l,est,cat,dd): _exec(f"INSERT INTO bill_templates(label,estimated_amount,category,due_day) VALUES({PH},{PH},{PH},{PH})",(l,est,cat,dd))
def del_bill_template(rid): _exec(f"DELETE FROM bill_templates WHERE id={PH}",(rid,))
def generate_bills_from_templates(month):
    existing=[r["label"] for r in get_bills(month)]; templates=_fetch("SELECT * FROM bill_templates WHERE active=1"); added=0
    for t in templates:
...
def set_payment(month,itype,iid,ilabel,amount,paid:bool):
    ...
    kw="EXCLUDED" if USE_PG else "excluded"
    _exec(f"INSERT INTO payments(month,item_type,item_id,item_label,amount,paid,paid_at) VALUES({PH},{PH},{PH},{PH},{PH},{PH},{PH}) ON CONFLICT(month,item_type,item_id) DO UPDATE SET paid={kw}.paid,paid_at={kw}.paid_at",(month,itype,iid,ilabel,amount,1 if paid else 0,paid_at))
