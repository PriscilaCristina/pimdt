...
# ── ATENÇÃO: init_db() foi movido para dentro de main() ─────────────
# Nunca chame db.init_db() aqui no nível de módulo — qualquer erro de
# conexão derrubaria o app inteiro antes de qualquer tela aparecer.

MN = {"01": "Janeiro", "02": "Fevereiro", "03": "Março", "04": "Abril",
...
# ══════════════════════════════════════════════════════════════════════════════
# PÁGINA DE ERRO DE CONEXÃO — exibida em vez de crashar
# ══════════════════════════════════════════════════════════════════════════════
def _db_error_page(error_msg: str):
    ...
    with col:
        ...
        <div style="text-align:center;padding:40px 0 24px">
          ...
          <h2 style="color:#1a2332;font-size:22px;font-weight:700">Erro de Conexão com o Banco</h2>
          <p style="color:#6b7280;font-size:13px">O app não conseguiu conectar ao banco de dados.</p>
        </div>
        ...
        st.markdown("""
        <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:12px;padding:20px 24px;margin:16px 0">
          <div style="font-weight:700;color:#92400e;margin-bottom:12px">📋 Como corrigir o DATABASE_URL</div>
          <ol style="color:#92400e;font-size:13px;line-height:2;padding-left:18px">
            <li>Abra o <b>Supabase Dashboard</b> → seu projeto</li>
            <li>Vá em <b>Project Settings → Database</b></li>
            <li>Copie a <b>Connection String (URI)</b></li>
            <li>Cole nos <b>Streamlit Secrets</b> como:<br>
              <code style="background:#fff;border:1px solid #fcd34d;border-radius:4px;padding:2px 8px;font-size:12px">DATABASE_URL = "postgresql://postgres:SENHA@db.REF.supabase.co:5432/postgres"</code>
            </li>
          </ol>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:16px 20px;margin-bottom:16px">
          <div style="font-weight:700;color:#991b1b;margin-bottom:8px">⚠️ Erro comum: API Key ≠ Senha do Banco</div>
          <div style="font-size:13px;color:#991b1b;line-height:1.8">
            A <b>API Key do Supabase</b> (começa com <code>eyJhbGc...</code>) é para a REST API.<br>
            Ela <b>NÃO funciona</b> como senha no DATABASE_URL.<br><br>
            Use a <b>Database Password</b> — é a senha que você criou ao criar o projeto.<br>
            Se esqueceu: <b>Supabase → Project Settings → Database → Reset Password</b>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:16px 20px">
          <div style="font-weight:700;color:#166534;margin-bottom:8px">💡 Alternativa: rodar sem banco (SQLite local)</div>
          <div style="font-size:13px;color:#166534">
            Se não quiser usar Supabase, <b>remova o DATABASE_URL</b> dos Secrets.<br>
            O app vai usar SQLite automático — dados ficam no servidor Streamlit Cloud
            (resetam em reinicializações, mas funciona para testes).
          </div>
        </div>
        """, unsafe_allow_html=True)
...
def login_page():
    ...
    with col:
        ...
        if st.button("Entrar →", use_container_width=True):
            secret_hash = ""
            try:
                secret_hash = st.secrets.get("PASSWORD_HASH", "")
            except Exception:
                pass
...
def sidebar(month):
    with st.sidebar:
        ...
        with c1:
            if st.button("◀", key="p"):
                st.session_state.month = prev(month)
                st.rerun()
        with c2:
        ...
        with c3:
            if st.button("▶", key="n"):
                st.session_state.month = nxt(month)
                st.rerun()
        ...
        if db.USE_PG:
            ...
            st.markdown('<div style="font-size:10px;color:#f59e0b;text-align:center;padding:2px 0">🟡 SQLite local</div>', unsafe_allow_html=True)
        st.divider()
...
def _run_diag(api_key, month, t_inc, t_gasto, t_debt, ef, ext, fix, subs, debts):
    if not api_key:
        st.warning("Configure a chave API em ⚙️ Configurações.")
        return
    import anthropic as _ant
    from collections import defaultdict
    cat_g = defaultdict(float)
    for r in fix:
        cat_g[r["category"]] += r["amount"]
    for r in ext:
        cat_g[r["category"]] += r["amount"]
    for r in subs:
        if r["active"]:
    ...
    with st.spinner("Analisando..."):
        try:
            ...
            st.session_state[f"diag_{month}"] = r.content[0].text
            st.rerun()
...
def _fluxo_ciclo(month, ciclo):
    inc = db.get_income(month)
    fix = db.get_fixed(month)
    bills = db.get_bills(month)
    subs = db.get_subs()
    cc_total = db.cc_total_month(month)
    debts = db.get_debts()
    invs = db.get_investments()
    ins = db.get_insurance()

    def _in(day):
        return day <= 15 if ciclo == 15 else day > 15
    ...
    for r in fix:
        if _in(r["due_day"]):
            gasto_items.append(("fixed", r["id"], r["label"], r["amount"], r["due_day"], "Gasto Fixo", r["category"]))
    for r in bills:
        if _in(r["due_day"]):
            gasto_items.append(("bill", r["id"], r["label"], r["amount"], r["due_day"], "Conta", r["category"]))
    for r in subs:
        if r["active"] and _in(r["billing_day"]):
            gasto_items.append(("sub", r["id"], r["label"], r["amount"], r["billing_day"], "Assinatura", r["category"]))
    if ciclo == 30:
        if cc_total > 0:
    ...
    for d in debts:
        dd = d.get("due_day", 30)
        if _in(dd):
            gasto_items.append(("debt", d["id"], d["label"], d["monthly_payment"], dd, "Dívida", ""))
    for i in ins:
        dd = i.get("due_day", 30)
        if _in(dd):
            gasto_items.append(("ins", i["id"], f"Seguro: {i['label']}", i["monthly_cost"], dd, "Seguro", ""))
    inv_this = next((r for r in invs if r["month"] == month), None)
    if inv_this:
        dd = inv_this.get("due_day", 30)
        if _in(dd):
            gasto_items.append(("inv", inv_this["id"], f"Investimento ({inv_this['investment_type']})", float(inv_this["amount_added"]), dd, "Investimento", ""))
                ...
                for (itype, iid, ilabel, iamt, iday, icat, isubcat) in gasto_items:
                    ...
                    if can_move and iid:
                        if c3.button(f"→{other}", key=f"mv_{itype}_{iid}_{ciclo}", help=f"Mover para dia {other}"):
                            if itype == "fixed":
                                db.update_fixed(iid, ilabel, iamt, other, isubcat or "Outros")
                            elif itype == "bill":
                                db.upsert_bill(month, ilabel, iamt, isubcat or "Utilidades", other)
                            elif itype == "sub":
                                db._exec(f"UPDATE subscriptions SET billing_day={db.PH} WHERE id={db.PH}", (other, iid))
                            elif itype == "debt":
                                db.update_debt_due_day(iid, other)
                            elif itype == "ins":
                                db.update_insurance_due_day(iid, other)
                            elif itype == "inv":
                                db._exec(f"UPDATE investments SET due_day={db.PH} WHERE id={db.PH}", (other, iid))
                            st.rerun()
...
def tab_painel(month):
    inc = db.get_income(month)
    fix = db.get_fixed(month)
    ext = db.get_extras(month)
    subs = db.get_subs()
    cc = db.cc_total_month(month)
    goals = db.get_goals()
    ef = float(db.get_ef(month))
    ef_target = float(db.get_config("ef_target", "0") or 0)
    debts = db.get_debts()
    bills = db.get_bills(month)
    ...
    invs_all = db.get_investments()
    inv_this = next((float(r["amount_added"]) for r in invs_all if r["month"] == month), 0.0)
    t_gasto = t_fix + cc + t_ext + t_subs + t_bills
    sobra = t_inc - t_gasto - t_debt
    payments = db.get_payments(month)
    t_paid = sum(p["amount"] for p in payments if p["paid"])
    ...
    with left:
        ...
        for due in [15, 30]:
            ...
            for r in sub:
                ...
                if c3.button("✎", key=f"ei{r['id']}"):
                    st.session_state[f"ei{r['id']}"] = True
                if c4.button("✕", key=f"di{r['id']}"):
                    db.del_income(r["id"])
                    st.rerun()
                if st.session_state.get(f"ei{r['id']}"):
                    with st.form(f"eif{r['id']}"):
                        nl = st.text_input("Descrição", r["label"])
                        na = st.number_input("Valor", value=float(r["amount"]), step=10.)
                        nd = st.selectbox("Dia", [15, 30], index=0 if r["due_day"] == 15 else 1)
                        if st.form_submit_button("Salvar"):
                            db.update_income(r["id"], nl, na, nd)
                            del st.session_state[f"ei{r['id']}"]
                            st.rerun()
        with st.expander("+ Adicionar entrada"):
            with st.form("add_inc"):
                l = st.text_input("Descrição", "", placeholder="Ex: Salário…")
                a = st.number_input("Valor", min_value=0., step=50.)
                d = st.selectbox("Dia", [15, 30])
                if st.form_submit_button("Adicionar"):
                    if l and a > 0:
                        db.add_income(month, l, a, d)
                        st.rerun()
        ...
        with st.expander("Atualizar reserva"):
            with st.form("ef_form"):
                nb = st.number_input("Saldo (R$)", value=float(ef), step=100.)
                nt = st.number_input("Meta (R$)", value=float(ef_target or t_gasto * 6), step=500.)
                if st.form_submit_button("Salvar"):
                    db.set_ef(month, nb)
                    db.set_config("ef_target", str(nt))
                    st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with right:
        ...
        for due in [15, 30]:
            ...
            for r in sub:
                ...
                if c3.button("✎", key=f"ef_{r['id']}"):
                    st.session_state[f"ef_{r['id']}"] = True
        ...
        with st.expander("+ Adicionar gasto fixo"):
            with st.form("add_fix"):
                l = st.text_input("Descrição", "", placeholder="Ex: Aluguel…")
                a = st.number_input("Valor", min_value=0., step=10.)
                d = st.selectbox("Venc.", [15, 30])
                ca = st.selectbox("Categoria", CATS)
                if st.form_submit_button("Adicionar"):
                    if l and a > 0:
                        db.add_fixed(month, l, a, d, ca)
                        st.rerun()
        ...
        try:
            api_d = api_d or st.secrets.get("ANTHROPIC_API_KEY", "")
        except Exception:
            pass
...
def tab_contas(month):
    ...
    templates = db.get_bill_templates()
    bills = db.get_bills(month)
    t_bills = sum(r["amount"] for r in bills)
        ...
        if bills:
            for r in bills:
                ...
                if c4.button("✅" if paid else "⬜", key=f"bp{r['id']}"):
                    db.set_payment(month, "bill", r["id"], r["label"], r["amount"], not paid)
                    st.rerun()
    ...
    with right:
        ...
        with st.form("add_bm"):
            l = st.text_input("Conta", "", placeholder="Ex: Conta de Luz")
            a = st.number_input("Valor (R$)", min_value=0., step=1.)
...
def tab_planilha(month):
    ...
    fix = db.get_fixed(month)
    bills = db.get_bills(month)
    cc_its = db.cc_items_month(month)
    subs = [r for r in db.get_subs() if r["active"]]
    ext = db.get_extras(month)
    debts = db.get_debts()
    ins = db.get_insurance()
    invs_all = db.get_investments()
    ...
    t_paid = sum(x[3] for x in all_exp if db.is_paid(month, x[0], x[1]))
    t_pend = t_exp - t_paid
    ...
    with left:
        ...
        for (itype, iid, ilabel, iamt, iday, icat) in all_exp:
            ...
            if c4.button("✓" if not paid else "↩", key=f"pay_{itype}_{iid}"):
                db.set_payment(month, itype, iid, ilabel, iamt, not paid)
                st.rerun()
    ...
    with right:
        ...
        for r in inc:
            ...
            if st.button("✓" if not paid else "↩", key=f"pay_inc_{r['id']}"):
                db.set_payment(month, "income", r["id"], r["label"], r["amount"], not paid)
                st.rerun()
...
def tab_variavel(month):
    ...
    with sub1:
        items_m = db.cc_items_month(month)
        all_cc = db.get_cc_all()
        total_m = db.cc_total_month(month)
        ...
        with right:
            ...
            with st.form("add_cc"):
                l = st.text_input("Descrição", "", placeholder="Ex: Notebook…")
                a = st.number_input("Valor total (R$)", min_value=0., step=10., key="cc_valor")
                n = st.number_input("Parcelas", min_value=1, max_value=60, value=1, key="cc_parcelas")
                sm = st.text_input("Mês inicial", value=month)
                cn = st.text_input("Cartão", "Cartão Principal")
                submitted = st.form_submit_button("Adicionar", use_container_width=True)
                if submitted:
                    if l and a > 0:
                        db.add_cc(l, a, int(n), sm, cn)
                        st.rerun()
            # Preview fora do form usando session state para não dar NameError
            a_val = st.session_state.get("cc_valor", 0.0)
            n_val = st.session_state.get("cc_parcelas", 1)
            ...
            if st.button("Processar com IA", key="proc_f", use_container_width=True):
                if api_key and fatura:
                    _parse_fatura(api_key, fatura, month, card_n)
                elif not api_key:
                    st.warning("Configure a chave API.")
                else:
                    st.warning("Cole o texto.")
            st.markdown('</div>', unsafe_allow_html=True)

    with sub2:
        saidas = db.get_extras(month, expense_type='saida')
        ...
        with right:
            ...
            with st.form("add_saida"):
                l = st.text_input("Descrição", "", placeholder="Ex: Cinema, Restaurante…")
                a = st.number_input("Valor (R$)", min_value=0., step=5.)
                ca = st.selectbox("Categoria", CATS)
                pm = st.selectbox("Pagamento", PAY)
                if st.form_submit_button("Registrar", use_container_width=True):
                    if l and a > 0:
                        db.add_extra(month, l, a, ca, pm, 'saida')
                        st.rerun()
                    else:
    ...
    with sub3:
        extras = db.get_extras(month, expense_type='extra')
        t_extras = sum(r["amount"] for r in extras)
        ...
        with right:
            ...
            with st.form("add_extra"):
                l = st.text_input("Descrição", "", placeholder="Ex: Padaria, Farmácia…")
                a = st.number_input("Valor (R$)", min_value=0., step=5.)
                ca = st.selectbox("Categoria", CATS)
                pm = st.selectbox("Método", ["PIX", "Dinheiro", "Transferência", "Débito", "Outro"])
                if st.form_submit_button("Registrar", use_container_width=True):
                    if l and a > 0:
                        db.add_extra(month, l, a, ca, pm, 'extra')
                        st.rerun()
                    else:
                        st.warning("Preencha os campos.")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with sub4:
        ...
        with right:
            ...
            with st.form("add_sub"):
                l = st.text_input("Serviço", "", placeholder="Ex: Netflix…")
                a = st.number_input("Valor/mês", min_value=0., step=1.)
    ...
    try:
        ...
        for it in items:
            if "label" in it and "total_amount" in it:
                db.add_cc(it["label"], float(it["total_amount"]), int(it.get("installments", 1)), it.get("start_month", month), card_name)
                count += 1
        st.success(f"✅ {count} item(s) importado(s)!")
        st.rerun()
    except Exception as e:
        st.error(f"Erro: {e}")
        ...
        with right:
            ...
            with st.form("inv_cfg"):
                ...
                if st.form_submit_button("Atualizar", use_container_width=True):
                    db.set_config("inv_contrib", str(nc))
                    db.set_config("inv_rate", str(nr))
                    st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            if ins:
                for r in ins:
                    ...
                    if c2.button("✅" if paid else "⬜", key=f"pin{r['id']}"):
                        db.set_payment(month, "ins", r["id"], r["label"], r["monthly_cost"], not paid)
                        st.rerun()
                    other_d = 15 if r.get("due_day", 30) == 30 else 30
                    if c3.button(f"→{other_d}", key=f"mvin{r['id']}"):
                        db.update_insurance_due_day(r["id"], other_d)
                        st.rerun()
                    if c4.button("✕", key=f"din{r['id']}"):
                        db.del_insurance(r["id"])
                        st.rerun()
            else:
            ...
            with st.form("add_ins"):
                ...
                if st.form_submit_button("Cadastrar", use_container_width=True):
                    if l and mc > 0:
                        ...
                        db.add_insurance(l, pv, mc, cv, int(ciclo_seg), notes)
                        st.rerun()
                    else:
                        st.warning("Preencha nome e custo.")
            st.markdown('</div>', unsafe_allow_html=True)
...
def tab_dividas():
    debts = db.get_debts()
    t_rem = sum(r["remaining_amount"] for r in debts)
    t_mp = sum(r["monthly_payment"] for r in debts)
        ...
        if debts:
            for r in debts:
                ...
                if ca.button("Salvar", key=f"dsr{r['id']}"):
                    db.update_debt_remaining(r["id"], nv)
                    st.rerun()
                other_d = 15 if r.get("due_day", 30) == 30 else 30
                if cb.button(f"→{other_d}", key=f"mvd{r['id']}"):
                    db.update_debt_due_day(r["id"], other_d)
                    st.rerun()
    ...
    with right:
        ...
        with st.form("add_debt"):
            l = st.text_input("Descrição", "", placeholder="Ex: Empréstimo…")
            total = st.number_input("Valor original (R$)", min_value=0., step=100.)
            rem = st.number_input("Saldo devedor (R$)", min_value=0., step=100.)
            mp = st.number_input("Parcela mensal (R$)", min_value=0., step=50.)
            ir = st.number_input("Juros mensais (%)", min_value=0., step=0.1)
            ciclo_d = st.selectbox("Ciclo", [30, 15])
            nt = st.text_input("Obs.", "")
            if st.form_submit_button("Cadastrar", use_container_width=True):
                if l and total > 0 and mp > 0:
                    db.add_debt(l, total, rem or total, mp, ir, int(ciclo_d), nt)
                    st.rerun()
                else:
                    st.warning("Preencha os campos.")
        st.markdown('</div>', unsafe_allow_html=True)
...
def tab_metas():
    goals = db.get_goals()
    t_tg = sum(g["target_amount"] for g in goals)
    t_cur = sum(g["current_amount"] for g in goals)
    ...
    with right:
        ...
        with st.form("add_goal"):
            l = st.text_input("Meta", "", placeholder="Ex: Viagem Europa…")
            tg = st.number_input("Valor alvo (R$)", min_value=0., step=500.)
            cur_g = st.number_input("Já tenho (R$)", min_value=0., step=100.)
            dl = st.text_input("Prazo", "")
            nt = st.text_input("Obs.", "")
            if st.form_submit_button("Cadastrar", use_container_width=True):
                if l and tg > 0:
                    db.add_goal(l, tg, cur_g, dl, nt)
                    st.rerun()
                else:
                    st.warning("Preencha os campos.")
        st.markdown('</div>', unsafe_allow_html=True)
...
def tab_assistente(month):
    ...
    t_ext = sum(r["amount"] for r in ext)
    t_subs = sum(r["amount"] for r in subs if r["active"])
    t_debt_m = sum(r["monthly_payment"] for r in debts)
    t_debt_t = sum(r["remaining_amount"] for r in debts)
...
Assinaturas: {s_txt} | Dívidas: {d_txt} | Metas: {g_txt}"""
    ...
    with st.spinner("Analisando..."):
        try:
            ...
            st.session_state[f"ai_{month}"] = r.content[0].text
            st.rerun()
        except Exception as e:
            st.error(f"Erro: {e}")
...
def _run_ai_q(api_key, question, month, t_inc, t_gasto, t_debt, t_inv, ef):
    ...
    with st.spinner("..."):
        try:
            ...
            st.session_state[f"ai_q_{month}"] = r.content[0].text
            st.rerun()
        except Exception as e:
            st.error(f"Erro: {e}")
...
def tab_config():
    ...
    with c2:
        ...
        cur = db.get_api_key()
        masked = f"sk-…{cur[-6:]}" if cur else "não configurada"
        ...
        if db.USE_PG:
            p = db._get_conn_params()
            masked_url = f"{p['user']}:***@{p['host']}:{p['port']}/{p['dbname']}" if p else "N/A"
            st.markdown(f'<div class="diag-ok">🟢 Conectado ao Supabase<br><code style="font-size:10px">{masked_url}</code></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="diag-warn">🟡 Usando SQLite local (dados não persistem entre deploys)</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown('<div class="card">', unsafe_allow_html=True)
        ...
        st.markdown(f'<div style="font-size:12px;color:#6b7280;line-height:2;margin-top:8px">Banco · <span style="color:#374151">{"Supabase (PostgreSQL)" if db.USE_PG else "SQLite local"}</span><br>IA · <span style="color:#374151">Claude (Anthropic)</span><br>Versão · <span style="color:#00c896;font-weight:600">8.1 — Família Peixoto</span></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        if st.button("🔄 Forçar reconexão ao banco", use_container_width=True):
            if "db_initialized" in st.session_state:
                del st.session_state["db_initialized"]
            st.rerun()
...
def main():
    ...
    # ── Inicializa DB uma única vez por sessão ──────────────────────
    ...
    sobra = t_inc - t_gas
    total_guardado = db.get_last_investment_total() + float(db.get_ef(month))
    cor = "#16a34a" if sobra >= 0 else "#ef4444"
