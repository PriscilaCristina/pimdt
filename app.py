...
st.set_page_config(page_title="Finanças Família", page_icon="💚", layout="wide", initial_sidebar_state="expanded")
db.init_db()
...
CATS=["Moradia","Transporte","Saúde","Educação","Alimentação","Lazer","Streaming","Serviços","Vestuário","Outros"]
PAY=["PIX","Dinheiro","Débito","Transferência","Outro"]

def R(v): return f"R$ {v:,.2f}".replace(",","X").replace(".",",").replace("X",".")
def ML(m): return f"{MN.get(m[5:],'?')} {m[:4]}"
def prev(m):
...
def prog_bar(pct,color="#00c896"):
    ...
    return f'<div style="background:#e5e7eb;border-radius:50px;height:8px;overflow:hidden;margin:6px 0"><div style="height:8px;border-radius:50px;background:{color};width:{pct}%"></div></div>'

def _auth_token():
    week=datetime.now().strftime("%Y%W")
    return hashlib.sha256(f"{db.get_config('password','')}{week}".encode()).hexdigest()[:24]
def _check_token(t): return t==_auth_token()
...
.stTextInput input:focus{border-color:var(--green)!important;box-shadow:0 0 0 3px rgba(0,200,150,.1)!important}
.stNumberInput>div>div{border:1.5px solid var(--border)!important;border-radius:8px!important;background:var(--white)!important}
.stNumberInput input{color:var(--body)!important;background:var(--white)!important;font-size:14px!important}
    ...
    with col:
        ...
        if st.button("Entrar →",use_container_width=True):
            ...
            if ok:
                ...
                st.rerun()
            else: st.error("Senha incorreta. Padrão: 1234")
        st.markdown('<p style="text-align:center;color:#d1d5db;font-size:11px;margin-top:14px">Dados seguros · Nenhum servidor externo</p>', unsafe_allow_html=True)
...
def sidebar(month):
    with st.sidebar:
        ...
        with c1:
            if st.button("◀",key="p"): st.session_state.month=prev(month); st.rerun()
        with c2:
        ...
        with c3:
            if st.button("▶",key="n"): st.session_state.month=nxt(month); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        st.divider()
        if st.button("↩ Desfazer última ação",use_container_width=True):
            ok,msg=db.undo_last(); (st.success if ok else st.warning)(msg); st.rerun()
...
def _run_diag(api_key,month,t_inc,t_gasto,t_debt,ef,ext,fix,subs,debts):
    ...
    total_debt=sum(r["remaining_amount"] for r in debts); sobra=t_inc-t_gasto-t_debt
    pct=(t_gasto/t_inc*100) if t_inc>0 else 0; ef_m=round(ef/t_gasto,1) if t_gasto>0 else 0
...
# ── Fluxo por ciclo ───────────────────────────────────────────────────────────
def _fluxo_ciclo(month, ciclo):
    inc=db.get_income(month); fix=db.get_fixed(month); bills=db.get_bills(month)
    subs=db.get_subs(); cc_total=db.cc_total_month(month)
    debts=db.get_debts(); invs=db.get_investments(); ins=db.get_insurance()
    ...
    if ciclo==30:
        if cc_total>0: gasto_items.append(("cc_total",0,"Cartão de Crédito",cc_total,30,"Cartão",""))
        for d in debts:
            dd=d.get("due_day",30)
            if _in(dd): gasto_items.append(("debt",d["id"],d["label"],d["monthly_payment"],dd,"Dívida",""))
        for i in ins:
            dd=i.get("due_day",30)
    ...
    for tab_c,ciclo,renda_items,gasto_items,t_renda,t_gastos,sobra in [(tab_c15,15,ri15,gi15,r15,g15,s15),(tab_c30,30,ri30,gi30,r30,g30,s30)]:
        with tab_c:
            ...
            with left:
                ...
                if renda_items:
                    for r in renda_items: st.markdown(f'<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border);font-size:13px"><span>{r["label"]}</span><span style="font-family:DM Mono,monospace;color:#16a34a">{R(r["amount"])}</span></div>', unsafe_allow_html=True)
                else: st.markdown(f'<p style="color:#6b7280;font-size:13px">Nenhuma renda para o dia {ciclo}.</p>', unsafe_allow_html=True)
            ...
            with right:
                ...
                for (itype,iid,ilabel,iamt,iday,icat,isubcat) in gasto_items:
                    ...
                    c1,c2,c3=st.columns([3.5,1.5,0.8])
                    c1.markdown(f'<div style="font-size:12px;font-weight:500">{ilabel}</div><span class="badge {badge}" style="font-size:9px">{icat}</span>', unsafe_allow_html=True)
                    c2.markdown(f'<span style="font-family:DM Mono,monospace;color:#ef4444;font-size:13px">{R(iamt)}</span>', unsafe_allow_html=True)
                    can_move=itype in ("fixed","bill","sub","debt","ins","inv")
                    if can_move and iid:
                        if c3.button(f"→{other}",key=f"mv_{itype}_{iid}_{ciclo}",help=f"Mover para dia {other}"):
                            ...
                            elif itype=="sub": db._exec(f"UPDATE subscriptions SET billing_day={db.PH} WHERE id={db.PH}",(other,iid))
                            elif itype=="debt": db.update_debt_due_day(iid,other)
                            elif itype=="ins": db.update_insurance_due_day(iid,other)
                            elif itype=="inv": db._exec(f"UPDATE investments SET due_day={db.PH} WHERE id={db.PH}",(other,iid))
                            st.rerun()
                ...
                st.markdown('</div>', unsafe_allow_html=True)
            cor="#16a34a" if sobra>=0 else "#ef4444"; bg="#f0fdf4" if sobra>=0 else "#fef2f2"; bdr="#bbf7d0" if sobra>=0 else "#fecaca"
            st.markdown(f'<div style="background:{bg};border:1px solid {bdr};border-top:3px solid {cor};border-radius:12px;padding:14px 18px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-size:9px;text-transform:uppercase;letter-spacing:2px;color:{cor};margin-bottom:4px">{"✅" if sobra>=0 else "⚠️"} Sobra do ciclo dia {ciclo}</div><div style="font-size:11px;color:{cor};opacity:.8">{R(t_renda)} renda — {R(t_gastos)} gastos</div></div><div style="font-size:24px;font-weight:700;color:{cor};font-family:DM Mono,monospace">{R(sobra)}</div></div>', unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════════════════
# PAINEL
# ══════════════════════════════════════════════════════════════════════════════
def tab_painel(month):
    inc=db.get_income(month); fix=db.get_fixed(month)
    ext=db.get_extras(month); subs=db.get_subs()
    cc=db.cc_total_month(month); goals=db.get_goals()
    ef=float(db.get_ef(month)); ef_target=float(db.get_config("ef_target","0") or 0)
    debts=db.get_debts(); bills=db.get_bills(month)
    ...
    t_bills=sum(r["amount"] for r in bills); t_debt=sum(r["monthly_payment"] for r in debts)
    t_ins=db.insurance_total()
    invs_all=db.get_investments(); inv_this=next((float(r["amount_added"]) for r in invs_all if r["month"]==month),0.0)
    t_gasto=t_fix+cc+t_ext+t_subs+t_bills; sobra=t_inc-t_gasto-t_debt
    payments=db.get_payments(month)
    t_paid=sum(p["amount"] for p in payments if p["paid"]); t_pend=(t_gasto+t_debt)-t_paid
    st.markdown(f"""<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0 14px">
      ...
      <p style="color:#6b7280;font-size:13px;margin-top:3px">Painel de {ML(month)} · ciclo 30/{prev(month)[5:]} → 29/{month[5:]}</p></div>
      <div style="display:flex;gap:8px">
        ...
        <div style="text-align:center;padding:8px 14px;background:{'#f0fdf4' if sobra>=0 else '#fef2f2'};border-radius:10px;border:1px solid {'#bbf7d0' if sobra>=0 else '#fecaca'}"><div style="font-size:9px;text-transform:uppercase;letter-spacing:1px;color:{'#16a34a' if sobra>=0 else '#dc2626'}">Saldo</div><div style="font-size:16px;font-weight:700;color:{'#16a34a' if sobra>=0 else '#dc2626'};font-family:DM Mono,monospace">{R(sobra)}</div></div>
    ...
    with cols[3]: st.markdown(f'<div class="mc"><div class="mc-val" style="color:#8b5cf6">{R(db.get_last_investment_total())}</div><div class="mc-label">Investido total</div></div>', unsafe_allow_html=True)
    ...
    with left:
        ...
        for due in [15,30]:
            ...
            for r in sub:
                ...
                if c4.button("✕",key=f"di{r['id']}"): db.del_income(r["id"]); st.rerun()
                if st.session_state.get(f"ei{r['id']}"):
                    with st.form(f"eif{r['id']}"):
                        ...
                        if st.form_submit_button("Salvar"): db.update_income(r["id"],nl,na,nd); del st.session_state[f"ei{r['id']}"]; st.rerun()
        with st.expander("+ Adicionar entrada"):
            with st.form("add_inc"):
                l=st.text_input("Descrição","",placeholder="Ex: Salário…"); a=st.number_input("Valor",min_value=0.,step=50.); d=st.selectbox("Dia",[15,30])
                if st.form_submit_button("Adicionar"):
                    if l and a>0: db.add_income(month,l,a,d); st.rerun()
        ...
        ef_pct=(ef/ef_target*100) if ef_target>0 else 0; mc=round(ef/t_gasto if t_gasto>0 else 0,1)
        ...
        with st.expander("Atualizar reserva"):
            with st.form("ef_form"):
                nb=st.number_input("Saldo (R$)",value=float(ef),step=100.); nt=st.number_input("Meta (R$)",value=float(ef_target or t_gasto*6),step=500.)
                if st.form_submit_button("Salvar"): db.set_ef(month,nb); db.set_config("ef_target",str(nt)); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        from collections import defaultdict
        ...
        if cat_t:
            ...
            lp=list(cat_t.keys()); vp=list(cat_t.values())
    ...
    with right:
        ...
        for due in [15,30]:
            ...
            for r in sub:
                ...
                if c4.button("✕",key=f"df_{r['id']}"): db.del_fixed(r["id"]); st.rerun()
                if st.session_state.get(f"ef_{r['id']}"):
                    with st.form(f"eff_{r['id']}"):
                        ...
                        nd=st.selectbox("Dia",[15,30],index=0 if r["due_day"]==15 else 1); nc=st.selectbox("Categoria",CATS,index=CATS.index(r["category"]) if r["category"] in CATS else 0)
                        if st.form_submit_button("Salvar"): db.update_fixed(r["id"],nl,na,nd,nc); del st.session_state[f"ef_{r['id']}"]; st.rerun()
        for label,value,color in [("💳 Cartão",cc,"#f59e0b"),("📱 Assinaturas",t_subs,"#8b5cf6"),("🏠 Contas",t_bills,"#ef4444"),("📤 Saídas/Extras",t_ext,"#6b7280"),("🛡️ Seguros",t_ins,"#3b82f6"),("📈 Investimento",inv_this,"#16a34a"),("⚠️ Dívidas/mês",t_debt,"#dc2626")]:
            if value>0: st.markdown(f'<div style="display:flex;justify-content:space-between;padding:5px 0;font-size:13px;border-top:1px dashed var(--border)"><span>{label}</span><span style="font-family:DM Mono,monospace;color:{color}">{R(value)}</span></div>', unsafe_allow_html=True)
        with st.expander("+ Adicionar gasto fixo"):
            with st.form("add_fix"):
                l=st.text_input("Descrição","",placeholder="Ex: Aluguel…"); a=st.number_input("Valor",min_value=0.,step=10.); d=st.selectbox("Venc.",[15,30]); ca=st.selectbox("Categoria",CATS)
                if st.form_submit_button("Adicionar"):
                    if l and a>0: db.add_fixed(month,l,a,d,ca); st.rerun()
        ...
        cd1.markdown('<div class="sec" style="margin-bottom:6px">Diagnóstico Financeiro</div>', unsafe_allow_html=True)
        api_d=os.environ.get("ANTHROPIC_API_KEY","") or db.get_api_key()
        try: api_d=api_d or st.secrets.get("ANTHROPIC_API_KEY","")
        except: pass
        if cd2.button("🔄",key="btn_diag",help="Atualizar IA"): _run_diag(api_d,month,t_inc,t_gasto,t_debt,ef,ext,fix,subs,debts)
        pct_g=(t_gasto+t_debt)/t_inc*100 if t_inc>0 else 0; ef_m2=round(ef/t_gasto,1) if t_gasto>0 else 0; total_dv=sum(r["remaining_amount"] for r in debts)
        if t_inc>0:
            if pct_g<=70: st.markdown(f'<div class="diag-ok">✅ Gastos: {pct_g:.0f}% da renda (ideal ≤70%)</div>', unsafe_allow_html=True)
        ...
        dk=f"diag_{month}"
        if dk in st.session_state: st.markdown(f'<div style="background:#f8fafc;border-left:3px solid #00c896;border-radius:8px;padding:10px 14px;margin-top:6px;font-size:13px;line-height:1.7">{st.session_state[dk].replace(chr(10),"<br>")}</div>', unsafe_allow_html=True)
        else: st.markdown('<p style="color:#9ca3af;font-size:11px;margin-top:4px">Clique 🔄 para análise personalizada</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        if goals:
            st.markdown('<div class="card card-green">', unsafe_allow_html=True)
            ...
            st.markdown('</div>', unsafe_allow_html=True)
    tab_fluxo_ciclos(month)

# ══════════════════════════════════════════════════════════════════════════════
# CONTAS FIXAS
# ══════════════════════════════════════════════════════════════════════════════
def tab_contas(month):
    db.generate_bills_from_templates(month)
    templates=db.get_bill_templates(); bills=db.get_bills(month); t_bills=sum(r["amount"] for r in bills)
    ...
    with left:
        ...
        if bills:
            for r in bills:
                paid=db.is_paid(month,"bill",r["id"])
                ...
                if c3.button("💾",key=f"bs{r['id']}"): db.upsert_bill(month,r["label"],nv,r["category"],r["due_day"]); st.rerun()
                if c4.button("✅" if paid else "⬜",key=f"bp{r['id']}"): db.set_payment(month,"bill",r["id"],r["label"],r["amount"],not paid); st.rerun()
        ...
        else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhuma conta. Adicione modelos →</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        if templates:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            ...
            for t in templates:
                ...
                if c3.button("✕",key=f"dtpl{t['id']}"): db.del_bill_template(t["id"]); st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
    with right:
        st.markdown('<div class="card card-red">', unsafe_allow_html=True)
        ...
        with st.form("add_bm"):
            ...
            ca=st.selectbox("Categoria",["Utilidades","Moradia","Comunicação","Outros"]); dd=st.number_input("Dia venc.",min_value=1,max_value=31,value=10)
            if st.form_submit_button("Adicionar",use_container_width=True):
                if l and a>0: db.upsert_bill(month,l,a,ca,int(dd)); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown('<div class="card">', unsafe_allow_html=True)
        ...
        with st.form("add_tpl"):
            ...
            ca=st.selectbox("Categoria",["Utilidades","Moradia","Comunicação","Outros"]); dd=st.number_input("Dia",min_value=1,max_value=31,value=10)
            if st.form_submit_button("Criar",use_container_width=True):
                if l: db.add_bill_template(l,a,ca,int(dd)); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════════════════
# PLANILHA DO MÊS
# ══════════════════════════════════════════════════════════════════════════════
def tab_planilha(month):
    inc=db.get_income(month); fix=db.get_fixed(month); bills=db.get_bills(month)
    cc_its=db.cc_items_month(month); subs=[r for r in db.get_subs() if r["active"]]
    ext=db.get_extras(month); debts=db.get_debts()
    ins=db.get_insurance(); invs_all=db.get_investments()
    inv_this=next((r for r in invs_all if r["month"]==month),None)
    ...
    for r in ext:    all_exp.append(("ext",r["id"],r["label"],r["amount"],0,f"Saída"))
    for d in debts:  all_exp.append(("debt",d["id"],d["label"],d["monthly_payment"],d.get("due_day",30),"Dívida"))
    ...
    t_paid=sum(x[3] for x in all_exp if db.is_paid(month,x[0],x[1])); t_pend=t_exp-t_paid
    ...
    with left:
        ...
        for (itype,iid,ilabel,iamt,iday,icat) in all_exp:
            paid=db.is_paid(month,itype,iid)
            ...
            if c4.button("✓" if not paid else "↩",key=f"pay_{itype}_{iid}"): db.set_payment(month,itype,iid,ilabel,iamt,not paid); st.rerun()
    ...
    with right:
        ...
        st.markdown('<div class="sec">Entradas</div>', unsafe_allow_html=True)
        for r in inc:
            paid=db.is_paid(month,"income",r["id"])
            ...
            if st.button("✓" if not paid else "↩",key=f"pay_inc_{r['id']}"): db.set_payment(month,"income",r["id"],r["label"],r["amount"],not paid); st.rerun()
...
# VARIÁVEL
# ══════════════════════════════════════════════════════════════════════════════
def tab_variavel(month):
    sub1,sub2,sub3,sub4=st.tabs(["💳 Cartão","🎬 Saídas","💸 Gastos Extras","📱 Assinaturas"])
    with sub1:
        items_m=db.cc_items_month(month); all_cc=db.get_cc_all(); total_m=db.cc_total_month(month)
        api_key=os.environ.get("ANTHROPIC_API_KEY","") or db.get_api_key()
        ...
        with left:
            ...
            if items_m:
                ...
                for it in items_m: st.markdown(f'<tr><td>{it["label"]}</td><td><span class="badge b-blue">{it["card"]}</span></td><td>{it["installment"]}</td><td style="font-family:DM Mono,monospace;color:#f59e0b">{R(it["monthly"])}</td></tr>', unsafe_allow_html=True)
            ...
            else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhuma cobrança.</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
            st.markdown('<div class="card">', unsafe_allow_html=True)
            ...
            if all_cc:
                for it in all_cc:
                    ...
                    if c3.button("✕",key=f"dcc{it['id']}"): db.del_cc(it["id"]); st.rerun()
            else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhuma compra.</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with right:
            ...
            with st.form("add_cc"):
                l=st.text_input("Descrição","",placeholder="Ex: Notebook…"); a=st.number_input("Valor total (R$)",min_value=0.,step=10.)
                n=st.number_input("Parcelas",min_value=1,max_value=60,value=1); sm=st.text_input("Mês inicial",value=month); cn=st.text_input("Cartão","Cartão Principal")
                if st.form_submit_button("Adicionar",use_container_width=True):
                    if l and a>0: db.add_cc(l,a,int(n),sm,cn); st.rerun()
            if int(n)>1 and a>0: st.markdown(f'<div style="padding:8px 12px;background:#fffbeb;border-radius:8px;font-size:12px;color:#92400e;margin-top:4px">{int(n)}× de {R(a/n)}</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
            st.markdown('<div class="card">', unsafe_allow_html=True)
            ...
            st.markdown('</div>', unsafe_allow_html=True)
    with sub2:
        saidas=db.get_extras(month,expense_type='saida'); t_saidas=sum(r["amount"] for r in saidas)
        ...
        st.info("🎬 Saídas de lazer e consumo (cinema, restaurante, compras…)")
        left,right=st.columns([1.2,1],gap="medium")
        with left:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            ...
            if saidas:
                ...
                for cat,items in grp.items():
                    ...
                    for r in items:
                        ...
                        if c3.button("✕",key=f"des{r['id']}"): db.del_extra(r["id"]); st.rerun()
                st.markdown(f'<div style="display:flex;justify-content:space-between;padding:8px 0 0;border-top:1px solid var(--border);font-size:13px;font-weight:600"><span>Total</span><span style="font-family:DM Mono,monospace">{R(t_saidas)}</span></div>', unsafe_allow_html=True)
            else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhuma saída.</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with right:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            ...
            with st.form("add_saida"):
                ...
                if st.form_submit_button("Registrar",use_container_width=True):
                    if l and a>0: db.add_extra(month,l,a,ca,pm,'saida'); st.rerun()
                    else: st.warning("Preencha os campos.")
            st.markdown('</div>', unsafe_allow_html=True)
    with sub3:
        extras=db.get_extras(month,expense_type='extra'); t_extras=sum(r["amount"] for r in extras)
        ...
        st.info("💸 PIX, dinheiro e transferências — gastos do dia a dia.")
        left,right=st.columns([1.2,1],gap="medium")
        with left:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            st.markdown('<div class="sec">Gastos</div>', unsafe_allow_html=True)
            if extras:
                st.markdown('<table class="tbl"><thead><tr><th>Descrição</th><th>Categoria</th><th>Método</th><th>Valor</th><th></th></tr></thead><tbody>', unsafe_allow_html=True)
                for r in extras: st.markdown(f'<tr><td>{r["label"]}</td><td><span class="badge b-gray">{r["category"]}</span></td><td>{r["payment_method"]}</td><td style="font-family:DM Mono,monospace">{R(r["amount"])}</td></tr>', unsafe_allow_html=True)
                ...
                for r in extras:
                    if st.button("✕",key=f"dex{r['id']}"): db.del_extra(r["id"]); st.rerun()
            else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhum gasto.</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with right:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            ...
            with st.form("add_extra"):
                ...
                if st.form_submit_button("Registrar",use_container_width=True):
                    if l and a>0: db.add_extra(month,l,a,ca,pm,'extra'); st.rerun()
                    else: st.warning("Preencha os campos.")
            st.markdown('</div>', unsafe_allow_html=True)
    with sub4:
        subs=db.get_subs(); t_subs=sum(r["amount"] for r in subs if r["active"])
        ...
        with left:
            ...
            if subs:
                ...
                for r in subs:
                    c1,c2,c3=st.columns([4,1,1]); c1.write(r["label"])
                    if c2.button("⏸" if r["active"] else "▶",key=f"ts{r['id']}"): db.toggle_sub(r["id"]); st.rerun()
                    if c3.button("✕",key=f"ds{r['id']}"): db.del_sub(r["id"]); st.rerun()
            else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhuma assinatura.</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with right:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            ...
            with st.form("add_sub"):
                ...
                ca=st.selectbox("Categoria",["Entretenimento","Educação","Software","Saúde","Outros"]); bd=st.number_input("Dia débito",min_value=1,max_value=31,value=1)
                if st.form_submit_button("Adicionar",use_container_width=True):
                    if l and a>0: db.add_sub(l,a,ca,int(bd)); st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
...
def _parse_fatura(api_key,text,month,card_name):
    ...
    try:
        ...
        st.success(f"✅ {count} item(s) importado(s)!"); st.rerun()
    except Exception as e: st.error(f"Erro: {e}")

# ══════════════════════════════════════════════════════════════════════════════
# INVESTIMENTOS & SEGUROS
# ══════════════════════════════════════════════════════════════════════════════
def tab_investimentos(month):
    invs=db.get_investments(); ins=db.get_insurance()
    annual_rate=float(db.get_config("inv_rate","12") or 12)
    monthly_contrib=float(db.get_config("inv_contrib","0") or 0)
    current_total=db.get_last_investment_total()
    ...
    with sub_inv:
        ...
        with cols[1]: st.markdown(f'<div class="mc"><div class="mc-val" style="color:#3b82f6">{R(db.investment_projection(current_total,monthly_contrib,annual_rate,5))}</div><div class="mc-label">Projeção 5 anos</div></div>', unsafe_allow_html=True)
        with cols[2]: st.markdown(f'<div class="mc"><div class="mc-val" style="color:#8b5cf6">{R(db.investment_projection(current_total,monthly_contrib,annual_rate,10))}</div><div class="mc-label">Projeção 10 anos</div></div>', unsafe_allow_html=True)
        with cols[3]: st.markdown(f'<div class="mc"><div class="mc-val" style="color:#f59e0b">{R(db.investment_projection(current_total,monthly_contrib,annual_rate,20))}</div><div class="mc-label">Projeção 20 anos</div></div>', unsafe_allow_html=True)
        ...
        with left:
            ...
            if invs:
                st.markdown('<table class="tbl"><thead><tr><th>Mês</th><th>Aportado</th><th>Origem</th><th>Ciclo</th><th>Total</th><th>Tipo</th></tr></thead><tbody>', unsafe_allow_html=True)
                for r in reversed(invs):
                    ...
                    ciclo_badge=f'<span class="badge b-{"15" if r.get("due_day",30)==15 else "30"}">Dia {r.get("due_day",30)}</span>'
                    st.markdown(f'<tr><td>{ML(r["month"])}</td><td style="font-family:DM Mono,monospace;color:#16a34a">{R(r["amount_added"])}</td><td>{src}</td><td>{ciclo_badge}</td><td style="font-family:DM Mono,monospace;font-weight:600">{R(r["total_accumulated"])}</td><td><span class="badge b-gray">{r["investment_type"]}</span></td></tr>', unsafe_allow_html=True)
                st.markdown('</tbody></table>', unsafe_allow_html=True)
                for r in invs:
                    if st.button(f"✕ {ML(r['month'])}",key=f"del_inv_{r['id']}"): db.del_investment(r["id"]); st.rerun()
            else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhum aporte.</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with right:
            st.markdown('<div class="card card-green">', unsafe_allow_html=True)
            st.markdown('<div class="sec">Registrar Aporte</div>', unsafe_allow_html=True)
            with st.form("add_inv"):
                sm=st.text_input("Mês (AAAA-MM)",value=month)
                aad=st.number_input("Valor aportado (R$)",min_value=0.,step=100.)
                source=st.selectbox("Origem",["Renda do mês (salário/rendimento)","Renda guardada (já estava investida)"])
                ciclo_inv=st.selectbox("Ciclo de pagamento",[30,15],help="Dia 15 ou 30?")
                tp=st.selectbox("Tipo",["Renda Fixa","Tesouro Direto","CDB","LCI/LCA","FII","Ações","Previdência","Cripto","Outros"])
                auto_total=current_total+aad if "Renda do mês" in source else current_total
                if aad>0: st.info(f"Total calculado: {R(auto_total)}")
                override=st.number_input("Total acumulado real (R$)",value=float(auto_total),step=100.)
                nt=st.text_input("Observações","")
                if st.form_submit_button("Salvar",use_container_width=True):
                    if aad>0: db.upsert_investment(sm,aad,float(override),tp,source,int(ciclo_inv),nt); st.rerun()
            ...
            with st.form("inv_cfg"):
                ...
                if st.form_submit_button("Atualizar",use_container_width=True):
                    db.set_config("inv_contrib",str(nc)); db.set_config("inv_rate",str(nr)); st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
    with sub_seg:
        t_ins=sum(r["monthly_cost"] for r in ins)
        ...
        with left:
            ...
            if ins:
                for r in ins:
                    paid=db.is_paid(month,"ins",r["id"])
                    c1,c2,c3,c4=st.columns([3.5,1,0.8,0.8])
                    c1.markdown(f'<div style="padding:6px 0"><div style="font-weight:600;font-size:13px">{r["label"]} {"✅" if paid else ""}</div><div style="font-size:11px;color:#6b7280">🏦 {r["provider"] or "?"} · 🛡️ {r["coverage"] or "?"}</div><div style="font-size:10px;color:#9ca3af">{r.get("notes","")}</div><div style="font-family:DM Mono,monospace;font-weight:700;color:#ef4444;font-size:13px">{R(r["monthly_cost"])}/mês</div><span class="badge b-{"15" if r.get("due_day",30)==15 else "30"}" style="font-size:9px">Dia {r.get("due_day",30)}</span></div>', unsafe_allow_html=True)
                    if c2.button("✅" if paid else "⬜",key=f"pin{r['id']}"): db.set_payment(month,"ins",r["id"],r["label"],r["monthly_cost"],not paid); st.rerun()
                    # toggle cycle
                    other_d=15 if r.get("due_day",30)==30 else 30
                    if c3.button(f"→{other_d}",key=f"mvin{r['id']}"): db.update_insurance_due_day(r["id"],other_d); st.rerun()
                    if c4.button("✕",key=f"din{r['id']}"): db.del_insurance(r["id"]); st.rerun()
            else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhum seguro.</p>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with right:
            st.markdown('<div class="card card-blue">', unsafe_allow_html=True)
            st.markdown('<div class="sec">Cadastrar Seguro</div>', unsafe_allow_html=True)
            with st.form("add_ins"):
                l=st.text_input("Tipo","",placeholder="Ex: Seguro de Vida")
                tp_ins=st.selectbox("Categoria",["Vida","Saúde","Carro","Casa/Residencial","Viagem","Outros"])
                pv=st.text_input("Seguradora","",placeholder="Ex: Bradesco Seguros")
                ap=st.text_input("Nº Apólice","")
                cv=st.text_input("Cobertura","",placeholder="Ex: R$ 500k")
                mc=st.number_input("Custo mensal (R$)",min_value=0.,step=10.)
                ciclo_seg=st.selectbox("Ciclo de pagamento",[30,15],help="Paga dia 15 ou 30?")
                nt=st.text_input("Obs.","")
                if st.form_submit_button("Cadastrar",use_container_width=True):
                    if l and mc>0:
                        notes=f"Tipo: {tp_ins}"+(f" · Apólice: {ap}" if ap else "")+(f" · {nt}" if nt else "")
                        db.add_insurance(l,pv,mc,cv,int(ciclo_seg),notes); st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
def tab_dividas():
    debts=db.get_debts(); t_rem=sum(r["remaining_amount"] for r in debts); t_mp=sum(r["monthly_payment"] for r in debts)
    st.markdown(f'<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0 14px"><div><h2 style="font-size:20px;font-weight:700;color:#1a2332;margin:0">Dívidas</h2><p style="color:#6b7280;font-size:12px;margin:3px 0 0">Acompanhe e zere em 12 meses</p></div><div style="font-size:20px;font-weight:700;color:#ef4444;font-family:DM Mono,monospace">{R(t_rem)}</div></div>', unsafe_allow_html=True)
    cols=st.columns(3,gap="small")
    ...
    with cols[2]:
        if debts: worst=max(debts,key=lambda x:x["interest_rate"]); st.markdown(f'<div class="mc"><div class="mc-val" style="color:#ef4444">{worst["interest_rate"]:.1f}%a.m.</div><div class="mc-label">Maior juros</div></div>', unsafe_allow_html=True)
        else: st.markdown('<div class="mc"><div class="mc-val" style="color:#16a34a">0%</div><div class="mc-label">Sem juros</div></div>', unsafe_allow_html=True)
    st.markdown('<div style="height:10px"></div>', unsafe_allow_html=True)
    left,right=st.columns([1.3,1],gap="medium")
    with left:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        ...
        if debts:
            for r in debts:
                ...
                if ca.button("Salvar",key=f"dsr{r['id']}"): db.update_debt_remaining(r["id"],nv); st.rerun()
                other_d=15 if r.get("due_day",30)==30 else 30
                if cb.button(f"→{other_d}",key=f"mvd{r['id']}"): db.update_debt_due_day(r["id"],other_d); st.rerun()
                if cc_col.button("✕",key=f"dd{r['id']}"): db.del_debt(r["id"]); st.rerun()
        else: st.markdown('<div class="diag-ok">🎉 Sem dívidas! Excelente!</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with right:
        st.markdown('<div class="card card-red">', unsafe_allow_html=True)
        ...
        with st.form("add_debt"):
            ...
            ir=st.number_input("Juros mensais (%)",min_value=0.,step=0.1); ciclo_d=st.selectbox("Ciclo",[30,15]); nt=st.text_input("Obs.","")
            if st.form_submit_button("Cadastrar",use_container_width=True):
                if l and total>0 and mp>0: db.add_debt(l,total,rem or total,mp,ir,int(ciclo_d),nt); st.rerun()
                else: st.warning("Preencha os campos.")
        st.markdown('</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
def tab_metas():
    goals=db.get_goals(); t_tg=sum(g["target_amount"] for g in goals); t_cur=sum(g["current_amount"] for g in goals)
    ...
    with left:
        ...
        if goals:
            for g in goals:
                ...
                color="#16a34a" if pct>=100 else "#3b82f6"; falta=max(0,g["target_amount"]-g["current_amount"])
                ...
                if cb.button("Salvar",key=f"gs{g['id']}"): db.update_goal(g["id"],nv); st.rerun()
                if cc_g.button("✕",key=f"gd{g['id']}"): db.del_goal(g["id"]); st.rerun()
        else: st.markdown('<p style="color:#6b7280;font-size:13px">Nenhuma meta.</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with right:
        st.markdown('<div class="card card-green">', unsafe_allow_html=True)
        ...
        with st.form("add_goal"):
            ...
            cur_g=st.number_input("Já tenho (R$)",min_value=0.,step=100.); dl=st.text_input("Prazo",""); nt=st.text_input("Obs.","")
            if st.form_submit_button("Cadastrar",use_container_width=True):
                if l and tg>0: db.add_goal(l,tg,cur_g,dl,nt); st.rerun()
                else: st.warning("Preencha os campos.")
        st.markdown('</div>', unsafe_allow_html=True)
...
# CONSULTOR IA
# ══════════════════════════════════════════════════════════════════════════════
def tab_assistente(month):
    st.markdown('<div style="text-align:center;padding:10px 0 20px"><h2 style="font-size:22px;font-weight:700;color:#1a2332;margin:0">Consultor Financeiro IA</h2><p style="color:#6b7280;font-size:13px;margin-top:5px">Análise direta — estilo <b>Bruno Perini</b> e <b>Thiago Nigro</b></p><div style="width:36px;height:3px;background:#00c896;border-radius:2px;margin:10px auto 0"></div></div>', unsafe_allow_html=True)
    api_key=os.environ.get("ANTHROPIC_API_KEY","") or db.get_api_key()
    try: api_key=api_key or st.secrets.get("ANTHROPIC_API_KEY","")
    except: pass
    if not api_key: st.warning("Configure a chave API em ⚙️ Configurações."); return
    inc=db.get_income(month); fix=db.get_fixed(month); ext=db.get_extras(month)
    subs=db.get_subs(); debts=db.get_debts(); invs=db.get_investments(); goals=db.get_goals()
    cc=db.cc_total_month(month); ef=float(db.get_ef(month))
    t_inc=sum(r["amount"] for r in inc); t_fix=sum(r["amount"] for r in fix)
    t_ext=sum(r["amount"] for r in ext); t_subs=sum(r["amount"] for r in subs if r["active"])
    t_debt_m=sum(r["monthly_payment"] for r in debts); t_debt_t=sum(r["remaining_amount"] for r in debts)
    t_inv=db.get_last_investment_total()
    ...
    if key in st.session_state:
        st.markdown(f'<div style="background:#f8fafc;border:1px solid #e5e7eb;border-left:4px solid #00c896;border-radius:12px;padding:22px 26px;margin-top:18px;font-size:14px;line-height:1.8;color:#374151;max-width:780px;margin-inline:auto"><div style="display:flex;align-items:center;gap:8px;margin-bottom:14px"><div style="width:30px;height:30px;border-radius:50%;background:#00c896;display:flex;align-items:center;justify-content:center;font-size:13px">💚</div><span style="font-weight:700;color:#1a2332">Consultor — {ML(month)}</span></div>{st.session_state[key].replace(chr(10),"<br>")}</div>', unsafe_allow_html=True)
        ...
        for i,q in enumerate(qs):
            if qc[i%2].button(q,key=f"q{i}"): _run_ai_q(api_key,q,month,t_inc,t_fix+cc+t_subs+t_ext,t_debt_t,t_inv,ef)
...
Assinaturas: {s_txt} | Dívidas: {d_txt} | Metas: {g_txt}"""
    ...
    with st.spinner("Analisando..."):
        try:
            r=client.messages.create(model="claude-opus-4-5",max_tokens=700,system=system,messages=[{"role":"user","content":prompt}])
...
def tab_config():
    ...
    with c1:
        ...
        with st.form("pwd"):
            cur=st.text_input("Senha atual",type="password"); n1=st.text_input("Nova senha",type="password"); n2=st.text_input("Confirmar",type="password")
            if st.form_submit_button("Alterar",use_container_width=True):
                if not db.check_pwd(cur): st.error("Senha incorreta.")
        ...
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown('<div class="sec">SQL para o Supabase</div>', unsafe_allow_html=True)
        st.markdown('<p style="font-size:12px;color:#6b7280;margin-bottom:6px">Cole no Supabase → SQL Editor → New query → Run:</p>', unsafe_allow_html=True)
        st.code(db.SUPABASE_SQL, language="sql")
        st.markdown('</div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="card card-green">', unsafe_allow_html=True)
        st.markdown('<div class="sec">Chave API Anthropic</div>', unsafe_allow_html=True)
        cur=db.get_api_key(); masked=f"sk-…{cur[-6:]}" if cur else "não configurada"
        ...
        with st.form("api"):
            ...
            if st.form_submit_button("Salvar",use_container_width=True):
                if k.startswith("sk-"): db.set_api_key(k); st.success("✅ Salva.")
                else: st.warning("Deve começar com 'sk-'")
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown('<div class="sec">Hash da senha (deploy)</div>', unsafe_allow_html=True)
        st.markdown('<p style="font-size:12px;color:#6b7280;margin-bottom:6px">Cole em PASSWORD_HASH nos Secrets do Streamlit Cloud:</p>', unsafe_allow_html=True)
        st.code(db.get_config("password",""),language=None)
        st.markdown('<div style="font-size:12px;color:#6b7280;line-height:2;margin-top:8px">Banco · <span style="color:#374151">SQLite local / Supabase (cloud)</span><br>IA · <span style="color:#374151">Claude (Anthropic)</span><br>Versão · <span style="color:#00c896;font-weight:600">8.0 — Família Peixoto</span></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
def main():
    ...
    if not st.session_state.auth: login_page(); return
    if "month" not in st.session_state: st.session_state.month=datetime.now().strftime("%Y-%m")
    month=sidebar(st.session_state.month)
    inc=db.get_income(month); fix=db.get_fixed(month); ext=db.get_extras(month); subs=db.get_subs(); cc=db.cc_total_month(month)
    t_inc=sum(r["amount"] for r in inc); t_gas=sum(r["amount"] for r in fix)+cc+sum(r["amount"] for r in ext)+sum(r["amount"] for r in subs if r["active"])
    sobra=t_inc-t_gas; total_guardado=db.get_last_investment_total()+float(db.get_ef(month)); cor="#16a34a" if sobra>=0 else "#ef4444"
    st.markdown(f"""<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0 14px;border-bottom:1px solid #e5e7eb;margin-bottom:16px">
      ...
      <div style="display:flex;gap:18px">
        ...
        <div style="width:1px;background:#e5e7eb"></div>
        <div style="text-align:center"><div style="font-size:9px;text-transform:uppercase;letter-spacing:1px;color:#6b7280">Total guardado</div><div style="font-size:14px;font-weight:700;color:#3b82f6;font-family:DM Mono,monospace">{R(total_guardado)}</div></div>
      </div></div>""", unsafe_allow_html=True)
    ...
    with t1: tab_painel(month)
    with t2: tab_contas(month)
    with t3: tab_planilha(month)
    with t4: tab_variavel(month)
    with t5: tab_investimentos(month)
    with t6: tab_dividas()
    with t7: tab_metas()
    with t8: tab_assistente(month)
    with t9: tab_config()
